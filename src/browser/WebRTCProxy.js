...
var MediaStream        = window.MediaStream           || window.mozMediaStream           || window.webkitMediaStream;
...
Session.prototype.call = function () {
	...
	function call() {
		...
		self.peerConnection = new PeerConnection({
			iceServers: [
				{
					url      : self.config.stun.host,
					username : self.config.stun.username,
					password : self.config.stun.password
				},
				{
					url        : self.config.turn.host,
					username   : self.config.turn.username,
					// credential : self.config.turn.username,
...
require("cordova/exec/proxy").add("WebRTCPlugin", module.exports);
